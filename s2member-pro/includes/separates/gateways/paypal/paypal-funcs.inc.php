...
*/
if (is_dir ($ws_plugin__s2member_pro_temp_dir = dirname (dirname (dirname (dirname (__FILE__)))) . "/functions/separates/gateways/paypal"))
	foreach (scandir ($ws_plugin__s2member_pro_temp_dir) as $ws_plugin__s2member_pro_temp_s) // Scan all files in this directory.
		if (preg_match ("/\.php$/", $ws_plugin__s2member_pro_temp_s) && preg_match ("/^paypal-/i", $ws_plugin__s2member_pro_temp_s))
