	...
	class c_ws_plugin__s2member_pro_menu_page_authnet_forms
	{
		public function __construct()
		{
			...
			for($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]; $n++)
			{
				...
				echo '<p>Currency: <select id="ws-plugin--s2member-pro-level'.$n.'-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/authnet-currencies.php"))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_authnetFormGenerate(\'level'.$n.'\');" /></p>'."\n";
			...
			echo '<p>Currency: <select id="ws-plugin--s2member-pro-modification-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/authnet-currencies.php"))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_authnetFormGenerate(\'modification\');" /></p>'."\n";
			...
			if(!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site())
			{
				...
				echo '<p>Currency: <select id="ws-plugin--s2member-pro-ccap-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/authnet-currencies.php"))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_authnetCcapFormGenerate();" /></p>'."\n";
			...
			echo '<p><em><strong>Cancellation Process:</strong> Very simple. A Member clicks a link to a Post/Page that contains a Cancellation Form you\'ve generated. The Member clicks the Submit button to confirm the cancellation. s2Member is notified silently behind-the-scenes, and will immediately cancel all future billing. s2Member will later terminate their account access, at the correct point in time. This works in conjunction with the s2Member Auto-EOT System. For further details, see: <strong>s2Member → Authorize.Net Options → EOT Behavior</strong>.</em></p>'."\n";
			...
			echo '<p>Currency: <select id="ws-plugin--s2member-pro-sp-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/authnet-currencies.php"))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_authnetSpFormGenerate();" /></p>'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			echo '<pre class="code">';
			echo '<code>';
			echo '[s2Member-Pro-AuthNet-Form rp="1" rt="M" rr="1" accept_coupons="1"]'."\n";
			echo "\t".'[s2Member-Pro-AuthNet-Form level="1" desc="Option 1 ($10.00)" ra="10.00" /]'."\n";
			echo "\t".'[s2Member-Pro-AuthNet-Form level="2" desc="Option 2 ($20.00)" ra="20.00" /]'."\n";
			echo "\t".'[s2Member-Pro-AuthNet-Form level="3" desc="Option 3 ($30.00)" ra="30.00" /]'."\n";
			echo '[/s2Member-Pro-AuthNet-Form]';
			echo '</code>';
			echo '</pre>'."\n";
			echo '</td>'."\n";
