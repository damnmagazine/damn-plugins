...
if(!class_exists('c_ws_plugin__s2member_pro_menu_page_stripe_forms'))
{
	/**
	 * Menu page for s2Member Pro (Stripe Forms page).
	 *
	 ...
	 * @since 140617
	 */
	class c_ws_plugin__s2member_pro_menu_page_stripe_forms
	{
		public function __construct()
		{
			...
			for($n = 1; $n <= $GLOBALS['WS_PLUGIN__']['s2member']['c']['levels']; $n++)
			{
				...
				echo '<p>Very simple. All you do is customize the form fields provided, for each Membership Level that you plan to offer. Then press (Generate Form Code). These special Stripe Forms are customized to work with s2Member seamlessly. Member accounts will be activated instantly, in an automated fashion. When a Member cancels their Membership, or fails to make payments on time, s2Member will automatically terminate their Membership privileges. s2Member listens to your Stripe Webhook integration; i.e., s2Member receives updates from Stripe behind-the-scene.</p>'."\n";
				...
				echo '<p>Currency: <select id="ws-plugin--s2member-pro-level'.$n.'-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_stripeFormGenerate(\'level'.$n.'\');" /></p>'."\n";
			...
			echo '<p>Currency: <select id="ws-plugin--s2member-pro-modification-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_stripeFormGenerate(\'modification\');" /></p>'."\n";
			...
			if(!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site())
			{
				...
				echo '<p>Currency: <select id="ws-plugin--s2member-pro-ccap-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_stripeCcapFormGenerate();" /></p>'."\n";
			...
			echo '</select></p>'."\n";
			echo '<p>Paid Subscr. ID: <input type="text" autocomplete="off" id="ws-plugin--s2member-pro-reg-link-subscr-id" value="" size="50" /> <a href="#" onclick="alert(\'The Customer\\\'s Paid Subscr. ID (aka: Stripe Subscription ID or Transaction ID) must be unique. This value can be obtained from inside your Stripe account. Each paying Customer MUST be associated with a unique Paid Subscr. ID. If the Customer is NOT associated with a Paid Subscr. ID, you will need to generate a unique value for this field on your own. But keep in mind, s2Member will be unable to maintain future communication with the Stripe Silent Post and ARB services if this value does not reflect a real Paid Subscr. ID, one that exists in your Stripe transaction log.\'); return false;" tabindex="-1">[?]</a></p>'."\n";
			echo '<p>Custom String Value: <input type="text" autocomplete="off" id="ws-plugin--s2member-pro-reg-link-custom" value="'.esc_attr($_SERVER['HTTP_HOST']).'" size="30" /> <a href="#" onclick="alert(\'A Paid Subscription is always associated with a Custom String that is passed through the custom=\\\'\\\''.c_ws_plugin__s2member_utils_strings::esc_js_sq(esc_attr($_SERVER['HTTP_HOST']), 3).'\\\'\\\' attribute of your Shortcode. This Custom Value, MUST always start with your domain name. However, you can also pipe delimit additional values after your domain, if you need to.\\n\\nFor example:\n'.c_ws_plugin__s2member_utils_strings::esc_js_sq(esc_attr($_SERVER['HTTP_HOST']), 3).'|cv1|cv2|cv3\'); return false;" tabindex="-1">[?]</a> <input type="button" value="Generate Access Link" onclick="ws_plugin__s2member_pro_stripeRegLinkGenerate();" /> <img id="ws-plugin--s2member-pro-reg-link-loading" src="'.esc_attr($GLOBALS['WS_PLUGIN__']['s2member']['c']['dir_url']).'/images/ajax-loader.gif" alt="" style="display:none;" /></p>'."\n";
			...
			echo '<p>Very simple. All you do is customize the form fields provided, for each Post/Page that you plan to sell. Then press (Generate Form Code). These special Stripe Forms are customized to work with s2Member seamlessly. You can even Package Additional Posts/Pages together into one transaction. <em><strong>Please note:</strong> forms are NOT saved here. This is only a Form Generator. Once you\'ve generated your Form, copy/paste it into any Post/Page you like. You\'ll want to provide your visitors with a link to the Post/Page where this Form is located.</em></p>'."\n";
			...
			echo '<p>I want to charge: $<input type="text" autocomplete="off" id="ws-plugin--s2member-pro-sp-amount" value="0.50" size="4" /> / <select id="ws-plugin--s2member-pro-sp-hours">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-sp-hours.php'))).'</select></p>'."\n";
			...
			echo '<p>Currency: <select id="ws-plugin--s2member-pro-sp-currency">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__s2member_pro_stripeSpFormGenerate();" /></p>'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			if(!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site())
			{
				...
				echo '<p>'.c_ws_plugin__s2member_utils_strings::highlight_php(file_get_contents(dirname(__FILE__).'/code-samples/stripe-s2p-v-query-ok-1.x-php')).'</p>'."\n";
				echo '<p>s2Member will only verify a query string for up to 10 seconds. After 10 seconds, <code>s2member_pro_stripe_s2p_v_query_ok()</code> will always return <code>false</code>, even if the integrity of the query string is valid. This prevents a Customer from bookmarking your Return URL; thereby causing duplicate commissions; in case you\'re using it for tracking purposes.</p>'."\n";
				...
				echo '<p><em>If it is your intention to allow Customers to bookmark your Custom Return URL, you can still do that. Just be aware that <code>s2member_pro_stripe_s2p_v_query_ok()</code> will return <code>false</code> after the first 10 seconds. If you want to verify after 10 seconds, you can pass a second argument to the function, like this:</em></p>'."\n";
				echo '<p>'.c_ws_plugin__s2member_utils_strings::highlight_php(file_get_contents(dirname(__FILE__).'/code-samples/stripe-s2p-v-query-ok-2.x-php')).'</p>'."\n";
			...
			echo '<ul class="ws-menu-page-li-margins">'."\n";
			echo '<li><code>accept="bitcoin"</code> Accept Bitcoin? Defaults to the value that you configure globally. Please see: <strong>s2Member → Stripe Options → Account Details → Accept Bitcoin via Stripe?</strong>. If you want to exclude Bitcoin as an accepted form of payment on a particular Pro-Form, you can set <code>accept=""</code> (i.e., to an empty string).</em></li>'."\n";
			echo '<li><code>accept_coupons="1"</code> Accept Coupons? Possible values: <code>0</code> = do NOT accept Coupons on this particular Form; <code>1</code> = DO accept Coupon Codes on this particular Form. <em><strong>NOTE:</strong> Coupon Codes MUST be configured with s2Member and NOT via the Stripe Dashboard. s2Member Pro-Forms currently do not support Stripe-generated coupon codes; only those configured with s2Member. Please see: <strong>s2Member → Pro Coupon Codes</strong> for configuration options.</em></li>'."\n";
			...
			echo '<li><code>cc="USD"</code> 3 character currency code. Not valid when <code>cancel="1"</code>. Please see <a href="http://www.s2member.com/r/stripe-currencies/" target="_blank" rel="external">this KB article at Stripe</a> for a <a href="http://www.s2member.com/r/stripe-currencies/" target="_blank" rel="external">list of supported currencies</a>. <em><strong>NOTE:</strong> no matter what currency code that you use, you should ALWAYS give s2Member the amount with decimals (i.e., the dollar amount). While it\'s true that some currency codes require a different denomoniation on the Stripe end of things; s2Member handles these conversions internally. In short, always provide the dollar amount; NOT the amount converted to cents.</em></li>'."\n";
			echo (!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site()) ? '<li><code>ccaps="music,videos"</code> A comma-delimited list of Custom Capabilities (if applicable). Only valid w/ Membership Level Access and/or Independent Custom Capabilities.</li>'."\n" : '';
			echo '<li><code>coupon="SAVE-10"</code> Default/pre-filled Coupon Code. This is optional, and the Coupon Code (if supplied) must exist in your Coupon Code configuration. <em><strong>NOTE:</strong> Coupon Codes MUST be configured with s2Member and NOT via the Stripe Dashboard. s2Member Pro-Forms currently do not support Stripe-generated coupon codes; only those configured with s2Member. Please see: <strong>s2Member → Pro Coupon Codes</strong> for configuration options.</em></li>'."\n";
			...
			echo '<li><code>reject_prepaid=""</code> By default, this is set to your global default option for either allowing or rejecting prepaid cards. However, if you want to override your global default configuration for a specific Pro-Form, you can set this to either <code>1</code> (reject) or <code>0</code> (allow). Not valid when <code>cancel="1"</code>. See also: <strong>s2Member → Stripe Options → Account Details → Reject or Allow Prepaid Cards?</strong>; i.e., your global default option.</li>'."\n";
			echo '<li><code>ra="0.50"</code> Regular, Buy Now, and/or Recurring Amount. Can also be <code>0.00</code> to provide free access. Not valid when <code>cancel="1"</code>.</li>'."\n";
			...
			echo '<pre class="code">';
			echo '<code>';
			echo '[s2Member-Pro-Stripe-Form rp="1" rt="M" rr="1" accept_coupons="1"]'."\n";
			echo "\t".'[s2Member-Pro-Stripe-Form level="1" desc="Option 1 ($10.00)" ra="10.00" /]'."\n";
			echo "\t".'[s2Member-Pro-Stripe-Form level="2" desc="Option 2 ($20.00)" ra="20.00" /]'."\n";
			echo "\t".'[s2Member-Pro-Stripe-Form level="3" desc="Option 3 ($30.00)" ra="30.00" /]'."\n";
			echo '[/s2Member-Pro-Stripe-Form]';
			echo '</code>';
			echo '</pre>'."\n";
			echo '</td>'."\n";
...
new c_ws_plugin__s2member_pro_menu_page_stripe_forms();
