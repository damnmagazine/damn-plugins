...
if(!class_exists("c_ws_plugin__s2member_pro_menu_page_clickbank_buttons"))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_clickbank_buttons
	{
		public function __construct()
		{
			...
			echo '<p>Very simple. All you do is customize the form fields provided, for each Post/Page that you plan to sell. Then press (Generate Button Code). These special ClickBank Buttons are customized to work with s2Member seamlessly. You can even Package Additional Posts/Pages together into one transaction. <em><strong>Please note:</strong> buttons are NOT saved here. This is only a Button Generator. Once you\'ve generated your Button, copy/paste it into your WordPress Editor, wherever you feel it would be most appropriate. If you lose your Button Code, you\'ll need to come back &amp; re-generate a new one.</em></p>'."\n";
			...
			echo '<p><input type="button" value="Generate Button Code" onclick="ws_plugin__s2member_pro_clickbankSpButtonGenerate();" /></p>'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			echo '<p><select id="ws-plugin--s2member-pro-sp-link-hours">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/clickbank-sp-hours.php"))).'</select> <input type="button" value="Generate Access Link" onclick="ws_plugin__s2member_pro_clickbankSpLinkGenerate();" /> <img id="ws-plugin--s2member-pro-sp-link-loading" src="'.esc_attr($GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["dir_url"]).'/images/ajax-loader.gif" alt="" style="display:none;" /></p>'."\n";
			...
			echo '<li><code>cbfid=""</code> Optional ClickBank <a href="http://www.s2member.com/r/clickbank-pitchplus/" target="_blank" rel="external">PitchPlus Upsell Flow</a> ID, which MUST correlate with a PitchPlus Upsell Flow you\'ve configured at ClickBank.com. <a href="#" onclick="alert(\'This MUST correlate with a PitchPlus Upsell Flow that you\\\'ve configured in your ClickBank account.\'); return false;" tabindex="-1">[?]</a></li>'."\n";
			echo '<li><code>cbur=""</code> Optional ClickBank <a href="http://www.s2member.com/r/clickbank-pitchplus/" target="_blank" rel="external">PitchPlus Upsell Flow</a> action (<code>a</code> accept or <code>d</code> decline), which MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. <a href="#" onclick="alert(\'This MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. Passing this parameter ALSO automatically enables the `cbf` session parameter; which is handled internally/automatically by s2Member Pro for you (if `cbur` is specified).\'); return false;" tabindex="-1">[?]</a></li>'."\n";
			echo '<li><code>cbf="auto"</code> Optional ClickBank <a href="http://www.s2member.com/r/clickbank-pitchplus/" target="_blank" rel="external">PitchPlus Upsell Flow</a> session ID, which MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. s2Member Pro handles this automatically when you define <code>cbur="a"</code> (or <code>cbur="d"</code>); and set this to <code>cbf="auto"</code> (recommended). <a href="#" onclick="alert(\'This MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. If you define `cbur`, and cbf=&quot;auto&quot;, this parameter is automatically set to the proper session ID for you; which is handled internally/automatically by s2Member Pro.\'); return false;" tabindex="-1">[?]</a></li>'."\n";
			...
			echo '<li><code>rt="D"</code> Regular Term. Only valid w/ Membership Level Access'.((is_multisite() && c_ws_plugin__s2member_utils_conds::is_multisite_farm() && !is_main_site()) ? '' : ' and/or Independent Custom Capabilities').'. Possible values: <code>D</code> = Days, <code>W</code> = Weeks, <code>M</code> = Months, <code>Y</code> = Years, <code>L</code> = Lifetime. Please note—ClickBank Recurring Products support only the following recurring intervals: weekly, bi-weekly, monthly, quarterly. <a href="#" onclick="alert(\'With the exception of Fixed-Term Access on Standard Products, this MUST correlate with a Product that you\\\'ve configured in your ClickBank account.\'); return false;" tabindex="-1">[?]</a></li>'."\n";
